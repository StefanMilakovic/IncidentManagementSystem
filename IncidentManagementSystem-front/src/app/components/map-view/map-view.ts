...
// Deklaracija za MarkerClusterGroup tip
declare module 'leaflet' {
...
export class MapView implements AfterViewInit, OnDestroy, OnChanges {
  ...
  public updateMarkers(): void {
    ...
    // Koristi L.markerClusterGroup direktno
    this.markerClusterGroup = L.markerClusterGroup({
      maxClusterRadius: 50,
      disableClusteringAtZoom: 13,
      spiderfyOnMaxZoom: true,
...
// Eksterni tip za MarkerClusterGroup
...
export class MapView implements AfterViewInit, OnDestroy, OnChanges {
  ...
  private clickHandler: any;
  private markerClusterGroup: any;
  private clusterPluginLoaded = false;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['incidents'] && this.map && this.clusterPluginLoaded) {
      this.updateMarkers();
    }
  }
  ...
  public updateMarkers(): void {
    ...
    // Provjeri da li je plugin učitan
    if (!this.clusterPluginLoaded || typeof (L as any).markerClusterGroup !== 'function') {
      console.error('❌ MarkerCluster not available, falling back to regular markers');
      this.addRegularMarkers();
      return;
    }

    try {
      // Kreiraj cluster grupu
  ...
  // Fallback metoda bez clusteringa
  private addRegularMarkers(): void {
    ...
    this.incidents.forEach(incident => {
      ...
      bounds.push(marker.getLatLng().toBounds(100));
    });
